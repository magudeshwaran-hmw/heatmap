...
async function ensurePhase2Tables() {
  try {
    ...
    // Seed the full built-in question bank (Python, SQL, Selenium, API Testing, …) so
    // the already-authored questions land in the DB automatically and show in /bank.
    try {
      await seedStaticQuestionBank();
    } catch (err) {
      console.error('⚠️ Seeding built-in question bank failed:', err.message);
    }
  } catch (err) {
...
// Auto-import the built-in question bank JSON (generated by scripts/gen-question-bank.cjs)
// into Postgres. Self-contained + de-duped by question text, so it is safe on every boot.
async function seedStaticQuestionBank() {
  let data;
  try { data = require('./src/data/questionBank.generated.json'); }
  catch { console.warn('ℹ️  No questionBank.generated.json (run: npm run gen:qbank) — skipping built-in seed.'); return; }
  const idx = (c) => (typeof c === 'number' ? (c >= 1 && c <= 4 ? c - 1 : c) : ({ A: 0, B: 1, C: 2, D: 3 }[String(c).trim().toUpperCase()] ?? 0));
  const diffOf = (d) => { const u = String(d || 'MEDIUM').toUpperCase(); return ['EASY', 'MEDIUM', 'HARD', 'SCENARIO'].includes(u) ? u : 'MEDIUM'; };
  let inserted = 0;
  for (const rec of (data.skills || [])) {
    const skill = rec.skill;
    for (const [level, sect] of [['beginner', rec.beginner], ['intermediate', rec.intermediate]]) {
      if (!sect) continue;
      const rows = [];
      (sect.mcq || []).forEach(m => rows.push({ qtype: 'mcq', qtext: m.question, options: JSON.stringify(m.options || []), correct: idx(m.correct), diff: diffOf(m.difficulty), payload: { question: m.question, options: m.options, correct: idx(m.correct), explanation: m.explanation || '' } }));
      (sect.toolId || []).forEach(t => rows.push({ qtype: 'toolId', qtext: t.description, options: '[]', correct: 0, diff: 'MEDIUM', payload: { description: t.description, correctAnswer: t.correctAnswer, keywords: t.keywords || [] } }));
      (sect.practical || []).forEach(p => rows.push({ qtype: 'practical', qtext: p.task, options: '[]', correct: 0, diff: 'MEDIUM', payload: { task: p.task, expectedKeywords: p.expectedKeywords || [], minLength: p.minLength || 30, sampleSolution: p.sampleSolution || '' } }));
      (sect.coding || []).forEach(c => rows.push({ qtype: 'coding', qtext: c.description || c.title, options: '[]', correct: 0, diff: 'HARD', payload: { title: c.title, description: c.description, examples: c.examples || [], testCases: c.testCases || [], starterCode: c.starterCode || {}, timeLimit: c.timeLimit || 30 } }));
      (sect.scenarios || []).forEach(s => rows.push({ qtype: 'scenarios', qtext: s.question, options: s.options ? JSON.stringify(s.options) : '[]', correct: s.correct != null ? idx(s.correct) : 0, diff: 'SCENARIO', payload: { question: s.question, minWords: s.minWords || 0, scoringKeywords: s.scoringKeywords || [], options: s.options, correct: s.correct != null ? idx(s.correct) : null } }));
      (sect.framework || []).forEach(s => rows.push({ qtype: 'framework', qtext: s.question, options: '[]', correct: 0, diff: 'SCENARIO', payload: { question: s.question, minWords: s.minWords || 0, scoringKeywords: s.scoringKeywords || [] } }));
      for (const it of rows) {
        if (!it.qtext) continue;
        const dup = await pool.query(`SELECT 1 FROM question_bank WHERE LOWER(skill_name)=LOWER($1) AND band=$2 AND COALESCE(qtype,'mcq')=$3 AND question_text=$4 LIMIT 1`, [skill, level, it.qtype, it.qtext]);
        ...
        await pool.query(
          ...
          [skill, level, it.diff, it.qtext, it.options, it.correct, it.payload.explanation || null, null, 1, 60, it.qtype, JSON.stringify(it.payload)]
        );
        inserted++;
      }
    }
  }
  if (inserted) console.log(`✅ Seeded ${inserted} built-in question-bank rows into the DB`);
}
